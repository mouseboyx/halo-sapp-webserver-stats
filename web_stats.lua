...
request_fail_after=request_fail_after*4;
ffi = require("ffi")
ffi.cdef [[
...
function OnDamageApplication(PlayerIndex, Causer, MetaID, Damage, HitString, Backtap)
	say_all(PlayerIndex..","..Causer..","..HitString..","..MetaID..","..Backtap)
	meta_id=tostring(MetaID)
    pi=tonumber(PlayerIndex);
    lastDamagedBy[pi]=MetaID;
    damageBacktap[pi]=Backtap
	...
	if (game_started==true) then
		if (killer~=-1 and killer~=0 and killer~=victim) then
            ...
            if (stop_http_requests==false) then
                say_all(killer_name);
               local url_text=domain_name.."halo.php?killer="..killer_name.."&killer_ip="..killer_ip.."&victim="..victim_name.."&victim_ip="..victim_ip.."&killed_by_weapon="..killed_by_weapon.."&body_part="..body_part.."&backtap="..was_backtap..server_key_request
